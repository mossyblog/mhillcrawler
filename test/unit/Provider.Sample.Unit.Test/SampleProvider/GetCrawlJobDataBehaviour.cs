...
{
    public class GetCrawlJobDataBehaviour : SampleProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(SampleConstants.KeyName.ApiKey, nameof(SampleCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task SampleCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Sample.Core.SampleCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
